...
async function startServer() {
  ...
  app.post("/api/auth/signup", async (req, res) => {
    try {
      ...
      await sessionsCollection.insertOne({ token, userId: id });
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      const token = crypto.randomBytes(32).toString("hex");
      await sessionsCollection.insertOne({ token, userId: user.id });
  ...
  app.post("/api/auth/google", async (req, res) => {
    try {
      ...
      // Check for mock testing credentials
      ...
      // Standard Google OAuth validation
      try {
...
"${text}"`,
      ...
      // Capture history record in MongoDB
  ...
  app.get("/api/history", async (req, res) => {
    ...
    const records = await translationsCollection.find({ userId: user.id }).toArray();
